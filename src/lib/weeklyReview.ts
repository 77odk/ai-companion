...
export interface WeeklyReview {
  ...
  /**
   * 批阅方式（TASK-UI3）：第一次批注时定下（immediate 立即回复 / sealed 封存慢信），之后锁定不可换。
   * 老数据没有此字段时按数据推导：有 myReply → immediate；有封存留言 → sealed。
   */
  reviewMode?: 'immediate' | 'sealed'
}
...
/** 一条消息的精简行：'8月18日 对方：…' / '8月18日 我：…'（内容压缩空白并截断到 60 字，防 prompt 膨胀） */
...
export const WEEKLY_SYSTEM_PROMPT =
  ...
  '信必须有完整的结尾：如果篇幅和完整结尾冲突，优先把结尾写完——宁可提前结束，也绝不能停在半句。标题用「」括起来，放在第一行。'
...
export function buildWeeklyPrompt(ctx: WeeklyPromptContext): string {
  ...
  lines.push(
    '【写作要求】',
    '以你的口吻，给对方写一篇 300-500 字的周记：',
    '1. 给这周取个标题，贴合本周发生的事（像《关于熬夜和米粉的一周》）。',
    '2. 把本周真实发生的、材料里提到的事写进去，贴着你们聊过的话写感受和心情，写心不写事——不流水账、不总结数据。',
    '3. 结尾一句关心或念叨对方的话。',
    '4. 只写上面材料里有依据的：材料里没发生的，不写、不编。',
    '5. 不刷 emoji（最多 1 个颜文字点缀）；不鸡汤、不喊口号。',
    '6. 篇幅要够：300-500 字，写真情实感、写够内心的份量，别凑字数、别空洞。' +
      '篇幅和完整结尾冲突时，优先把结尾写完：宁可提前结束，也绝不能停在半句。',
  )
  if (pending.length > 0) {
    lines.push(
      '7. 输出格式：第一行「标题」，下面接正文；正文之后另起一行写「回信：」，然后逐条回信——每条回信用一行「---」分隔，每条回信先简短提一句那条封存留言说了什么，再写完整的回信内容。',
    )
  } else {
    lines.push('7. 直接输出：第一行「标题」，下面接正文。')
  }
