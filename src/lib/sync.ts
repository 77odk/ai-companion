...
import { collectAllGenders, applyCloudGenders, type AIGender } from './storage.ts'
...
export interface SyncData {
  ...
  /** Context Meter：session → 当前上下文占用与最近一轮 usage；继续走既有全量 blob。 */
  contextMeters?: Record<string, ContextMeterState>
}
...
export function collectData(): SyncData {
  return {
    ...
    contextMeters: collectAllContextMeters(),
  }
}
// ---- 合并策略（纯函数，可单测） ----
/**
...
export function applyData(data: SyncData): void {
  ...
  applyCloudContextMeters(d.contextMeters)
