 ...
 */
const BUSY_INTENT_PREFIX = /(我去|我出去|我先|我得|我要去|等我|我忙一下|我去忙|我先忙|我忙|我去去就回|我处理|我收拾|我加会儿|我去处理|我去弄|(?:好|嗯|行|好的|好嘞|可以|成|没问题|ok|OK|嗯嗯)[，,。 ]{0,2}(?:这就|马上去|先去|去|要(?:去|忙|弄)))/

/**
 * 完整离开句式：前缀 + 1~18 字实际内容（不再强求"了/吧"结尾——2026-09-05 晚 七七拍板放宽：
 * 只要表达"去忙自己的事"就触发，好/嗯应承式也算）。
 * 两个变体：
 *  - A 强离开前缀：我去/我先/我得/我要去/等我/我忙/我去忙 等
 *  - B 应承式：好/嗯/行/OK + 这就/马上去/先去/去 + 动作（"好，这就去洗"）
 * 句末标点（。！？\n）处截停；内容至少 1 字（"我去"+逗号=惊叹感叹不触发）。
 */
const BUSY_INTENT_FULL_A = /(我去|我出去|我先|我得|我要去|等我|我忙一下|我去忙|我先忙|我忙|我去去就回|我处理|我收拾|我加会儿|我去处理|我去弄)[^。！？!?，,\n]{1,18}/
const BUSY_INTENT_FULL_B = /((?:好|嗯|行|好的|好嘞|可以|成|没问题|ok|OK|嗯嗯)[，,。 ]{0,2}(?:这就|马上去|马上去弄|先去|去|要(?:去|忙|弄)))[^。！？!?\n]{1,18}/

/**
 * 排除列表：句式命中但内容是奔向对方的（为对方做的事/继续跟对方互动），不触发。
 * TA 说"我去找你吧"是要继续聊，不该消失 4-5 分钟。
 * 2026-09-05 晚 加宽：给你/找你/陪你/等你/跟你/回你/问你/告诉你 等"伺候你/还在跟你互动"全排除。
 */
const BUSY_EXCLUDE = /(找你|接你|给你|陪你|来看你|去找你|等你|跟你|回你消息|问你|跟你说|告诉你|跟你商量|问问你|想想怎么|看看你怎么)/
...
export function matchBusyIntent(text: string): RegExpMatchArray | null {
  const t = String(text ?? '')
  // 中文句式 v2（2026-09-05 晚 放宽拍板）：A 强离开前缀优先，B 应承式兜底
  let m = t.match(BUSY_INTENT_FULL_A)
  if (!m) m = t.match(BUSY_INTENT_FULL_B)
  if (m) {
