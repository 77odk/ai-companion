...
const RECENT_N = 3
/**
 * 把时间差转成"刚刚/N分钟前/N小时前/N天前"。
 * ts 缺失或非法返回空串（调用方自行决定是否带时间前缀）。
 * 纯函数，可单测。
 */
export function formatAgo(ts: number | undefined, now: number = Date.now()): string {
 ...
 * 纯函数，可单测。
 */
export function buildSelfTimelineBlock(messages: TimelineMsg[], now: number = Date.now()): string {
  ...
  for (const m of recent) {
    ...
    const ago = formatAgo(m.ts, now)
    lines.push(`${ago ? `${ago} ` : ''}你说过：${text}`)
  }
