...
const TOPIC_RULES: Array<[RegExp, string]> = [
  ...
  [/工作|上班|下班|收工|公司|同事|加班|老板|出差|开会|项目|辞职|离职|入职|绩效|裁员|晚班|早班|夜班|白班|中班|轮班|倒班|值班|作息|通勤/, '工作'],
...
export function extractThinkBlocks(text: string): string {
  const t = String(text ?? '')
  if (!t) return ''
  const matches = t.match(/<think>([\s\S]*?)(?:<\/think>|$)/g)
  if (!matches || matches.length === 0) return ''
  return matches
    .map((m) => m.replace(/^<think>\s*/, '').replace(/\s*<\/think>$/, '').trim())
    .filter(Boolean)
    .join('\n---\n')
}
...
/**
 * 作息事实自动记忆（2026-09-09 七七拍板·作息自动记，补 detectPreferenceFact 的漏网）：
 * 「我上晚班」「我晚上十一点才下班」「我每天八点上班」这类稳定作息 → 保底提取存记忆。
 * 只做浅层正则，宁可漏不可错存：
 * - 一次性的不算（今天/明天/昨晚/待会……带这些词不记）
 * - 纯情绪/感叹不命中（正则要求 班次词 或 稳定频率/时段+时间+上下班动作）
 * - 命中返回整句原文（进库后注入侧统一转「对方」视角）
 */
const SCHEDULE_ONCE_RE = /今天|明天|昨天|今晚|明晚|昨晚|这次|待会|马上|一会儿|刚/
/** 时间里的数字：支持阿拉伯数字与汉字数字（八点/十一点/二十三点/两点半） */
const NUM = '[0-9一二三四五六七八九十两]{1,3}'
const SCHEDULE_PATTERNS: RegExp[] = [
  // 班次制：我上晚班 / 我是夜班 / 我晚班到十一点
  /我(?:是|上|上的是|上的|值|倒|做)?(?:早班|晚班|夜班|中班|白班)(?:到|上到|上至)?\s*点?/,
  // 我的班是白班 / 我班是晚班
  /我(?:的)?(?:班|工作)(?:是|上的是)(?:早班|晚班|夜班|中班|白班)/,
  // 稳定频率 + 时间 + 上下班/作息动作：我每天八点上班 / 我一般九点半睡觉
  new RegExp(`我(?:每天|平时|一般|通常|固定|都是)(?:早上|上午|中午|下午|晚上|凌晨)?${NUM}点(?:半|钟|左右)?[才就]?.{0,2}(?:上班|下班|收工|起床|睡觉|睡)`),
  // 时段 + 时间 + 动作（无频率词但带明确时段也认）：我晚上十一点才下班 / 我早上六点半起床
  new RegExp(`我(?:早上|上午|中午|下午|晚上|凌晨)${NUM}点(?:半|钟|左右)?[才就]?.{0,2}(?:上班|下班|收工|起床|睡觉|睡)`),
]

export function detectScheduleFact(text: string): string | null {
  const t = String(text ?? '').trim()
  if (!t || t.length > 60) return null
  if (SCHEDULE_ONCE_RE.test(t)) return null // 一次性语境：今天晚班/明天早走都不算稳定作息
  for (const re of SCHEDULE_PATTERNS) {
    const m = re.exec(t)
    if (m && m[0].length >= 3) return t
  }
  return null
}
