...
assert.match(rolePicker, /<span className="role-card-name">自定义 TA<\/span>/)
assert.doesNotMatch(rolePicker, /role-templates/)
assert.match(rolePicker, /if \(mode === 'current'\) \{[\s\S]*<AIDetail/)
assert.match(rolePicker, /title=\{setup\.kind === 'natural' \? '认识 TA' : '自定义 TA'\}/)
assert.match(rolePicker, /const valid = isNatural[\s\S]*\? true[\s\S]*form\.nickname\.trim\(\) !== '' && form\.personality\.trim\(\) !== ''/)
assert.match(rolePicker, /TA姓名[\s\S]*isNatural \? 'optional-mark' : 'required-mark'/)
...
assert.match(rolePicker, /还没想好？从一个人设模板开始/)
assert.match(rolePicker, /visibleTemplates\.map/)
assert.match(rolePicker, /TEMPLATE_FILTERS\.map/)
