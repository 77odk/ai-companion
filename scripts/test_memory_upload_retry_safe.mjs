...
console.log('\n[9] 本机时间偏差 >5 分钟：不能把同 payload 的旧云端记录当成同一次上传')
reset()
requests = []
getCount = 0
handler = async (req) => {
...
console.log('\n[10] 超出时间窗即使 payload 完全一致，也保持事件身份分离')
reset()
requests = []
getCount = 0
handler = async (req) => {
...
result = await retryPendingMemoryUploads('token-clock-skew-duplicate-payload', SID)
assert.equal(getCount, 2)
assert.equal(result.reconciled, 0)
assert.equal(result.uploaded, 1)
assert.equal(requests.filter((req) => req.method === 'POST').length, 1)
...
handler = async (req) => {
  if (req.method === 'GET') {
    ...
    return json({
      memories: [{
        ...cloudMemory(455, '记住这件事', 12 * 60 * 60 * 1000),
        source: '另一段旧原话',
      }],
    })
  }
