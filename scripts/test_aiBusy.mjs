...
// 不触发：半句话（无结尾词）
ok(containsBusyKeyword('我去洗碗'), '我去洗碗 → 触发（2026-09-05 晚放宽：表达离开即触发，不再等完整句式）')
ok(containsBusyKeyword('我先去洗个澡'), '我先去洗个澡 → 触发（放宽）')
ok(containsBusyKeyword('我去开个会'), '我去开个会 → 触发（放宽）')
ok(containsBusyKeyword('好，这就去洗'), '应承式：好，这就去洗 → 触发（放宽新增）')
ok(containsBusyKeyword('嗯，马上去弄'), '应承式：嗯，马上去弄 → 触发（放宽新增）')
ok(containsBusyKeyword('我处理点事'), '我处理点事 → 触发（放宽新增）')
ok(containsBusyKeyword('等我一下'), '等我一下 → 触发')
ok(!containsBusyKeyword('我去，这么贵'), '我去（惊叹感叹）→ 不触发')
ok(!containsBusyKeyword('我去想想怎么回你'), '我去想想怎么回你（还在想怎么互动）→ 不触发')
ok(!containsBusyKeyword('我等你回来'), '我等你回来（继续等对方）→ 不触发')
// 不触发：排除列表（为对方做的事）
...
ok(ctx.includes('[source=USER] USER 休息一会要去上班呢'), '含用户消息（标 USER）')
ok(ctx.includes('[source=SELF] 那 USER 去忙吧'), '含 TA 消息（标 SELF）')
ok(ctx.includes('[source=USER] USER 说 USER 现在休息一会'), '含最新用户消息')
ok(ctx.split('\n').length <= 3, '最多 3 条')
