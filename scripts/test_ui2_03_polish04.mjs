...
check('「翻开 →」主文字 ink（POLISH-05 去 CTA 感）+ 箭头 Accent', /\.mbp-open\s*\{[\s\S]{0,240}color: color-mix\(in srgb, var\(--ui2-text\)/.test(CSS) && /\.mbp-open span\s*\{[\s\S]{0,80}color: var\(--ta-accent\)/.test(CSS))
check('植物花枝 mbp-botanical（inline SVG 装帧）', /mbp-botanical/.test(Memory) && /\.mbp-botanical\s*\{/.test(CSS) && /<svg/.test(Memory))
check('书脊竖排 ELUVIN', /mbp-spine-brand/.test(Memory) && /writing-mode: vertical-rl/.test(CSS) && /ELUVIN/.test(Memory))
check('书脊小星芒 + 右侧竖排装帧字（装帧符号）', /mbp-spine-mark/.test(Memory) && /mbp-side-vertical/.test(Memory) && /writing-mode: vertical-rl/.test(CSS))
...
check('无网络图片/生成图', !/<img/.test(Memory) && !/url\(['"]?https?:/.test(CSS))
...
check('hover 微增强（botanical 显现）', /\.memory-book-portal:hover \.mbp-botanical[\s\S]{0,80}opacity: 1/.test(CSS))
