...
ok(withPersona.includes('不知道就说不知道'), '规矩1：不编不圆')
ok(withPersona.includes('像自然私聊'), '规矩2：像自然私聊')
ok(withPersona.includes('不用客服式套话'), '规矩2：公共层避免客服腔')
ok(withPersona.includes('可以有不同意见'), '规矩3：允许不同意见，不一味附和')
ok(withPersona.includes('自己的连续状态服从当前身份模式'), '规矩3：连续状态服从身份模式')
ok(withPersona.includes('别用空泛鼓励'), '规矩4：倾诉时不套空泛鼓励')
ok(withPersona.includes('递一个具体可接的话头'), '规矩4：冷淡时递钩子')
...
ok(noPersona.includes('你是这段关系里持续存在的 TA'), '无人设兜底身份在')
ok(noPersona.includes('【聊天规矩】'), '无人设也带聊天规矩')
ok(noPersona.includes('对方怎么称呼你，就自然接住这个称呼'), '兜底身份原文')
...
ok(buildSystemPrompt('你是温柔男友', '小忆', new Date(2026, 8, 4, 20, 0).getTime()).length < 1500, `提示词总长 <1500（含来源说明；得 ${buildSystemPrompt('你是温柔男友', '小忆', new Date(2026, 8, 4, 20, 0).getTime()).length}）`)
