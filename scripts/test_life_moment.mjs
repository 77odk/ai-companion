...
ok(withPersona.includes('像自然的私聊'), '规矩2：像自然的私聊')
ok(withPersona.includes('自然带一点你自己的连续状态'), '规矩3：保留连续状态（身份中性）')
ok(withPersona.includes('必须服从当前身份模式'), '规矩3：表达服从身份模式')
ok(withPersona.includes('递一个具体、可接的话头继续聊'), '规矩4：冷淡时递钩子')
