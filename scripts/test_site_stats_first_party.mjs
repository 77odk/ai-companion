...
console.log('\n[4] 打点挂在 App 挂载上，每次页面加载一次')
const appSrc = fs.readFileSync(path.join(process.cwd(), 'src/App.tsx'), 'utf8')
assert.match(appSrc, /pingSiteHit\(API_BASE\)/)
assert.match(appSrc, /useEffect\(\(\) => \{\s*pingSiteHit\(API_BASE\)\s*\}, \[\]\)/, '只在挂载时打一次')
...
console.log('\n[6] 行为：解析数字；失败返回 null；本地开发不打点')
const { fetchSiteStats, pingSiteHit } = await import('../src/lib/siteStats.ts')
...
pingSiteHit('https://api.example')   // Node 无 location → 视为本地开发，不打点
await new Promise((r) => setTimeout(r, 20))
globalThis.fetch = realFetch
assert.equal(pinged, 0, '本地开发不打点，避免污染线上数字')
