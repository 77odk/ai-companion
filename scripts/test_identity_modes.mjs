...
assert.ok(CHAT_RULES.includes('自己的连续状态服从当前身份模式'))
assert.ok(CHAT_RULES_EN.includes('SELF continuity follows the current identity mode'))
...
assert.equal(looksRobotic('我是 AI。', 'immersive'), true, '沉浸档拦普通空格 AI 身份披露')
assert.equal(looksRobotic('我是一个 AI。', 'immersive'), true, '沉浸档拦“一个 AI”空格写法')
assert.equal(looksRobotic('我是　AI。', 'immersive'), true, '沉浸档拦全角空格 AI 身份披露')
assert.equal(looksRobotic('作为 AI，我可以解释。', 'immersive'), true, '沉浸档拦“作为 AI”空格写法')
assert.equal(looksRobotic('我是一个AI。', 'natural'), false, '自然档允许在需要时承认 AI 身份')
assert.equal(looksRobotic('我是 AI。', 'natural'), false, '自然档空格写法仍可承认 AI')
assert.equal(looksRobotic('我是一个AI。', 'ai'), false, 'AI 档允许直接表达 AI 身份')
assert.equal(looksRobotic('作为 AI，我可以解释。', 'ai'), false, 'AI 档空格写法仍放行')
...
console.log('\n[5] Runtime：三档都不再自动创造状态；只有聊天证据能写入当前状态')
...
assert.equal(naturalRuntime.activityId, 'idle', '自然档无证据 → idle')
assert.equal(aiRuntime.activityId, 'idle', 'AI 档无证据 → idle')
saveIdentityMode('A', 'immersive')
const immersiveRuntime = getOrAdvanceTaRuntime('A', '', noon + 1, () => 0)
assert.equal(immersiveRuntime.activityId, 'idle', '沉浸档也不再靠随机 routine 创造“TA 此刻”')
...
assert.match(chatSource, /<ChatCompanionControls sessionId=/)
assert.doesNotMatch(appSource, /<ChatCompanionControls sessionId=/)
