...
check('含 spine 元素', /memory-book-portal-spine/.test(Memory))
check('含 cover / kicker / title / copy / open', ['memory-book-portal-cover', 'memory-book-portal-kicker', 'memory-book-portal-title', 'memory-book-portal-copy', 'memory-book-portal-open'].every((c) => Memory.includes(c)))
...
check('生产代码无 .memory-entry-media 渲染', !/memory-entry-media/.test(Memory) || /memory-entry-media/.test(Memory.replace(/\/\*[\s\S]*?\*\//g, '')) === false || true) // 只允许注释中出现
const codeWithoutComments = Memory.replace(/\/\*[\s\S]*?\*\//g, '')
check('JSX 中无 .memory-entry-media（注释外）', !/memory-entry-media/.test(codeWithoutComments))

console.log('[3] ui2.css 实体书规则')
check('含 .memory-book-portal 基础规则', /\.memory-book-portal\s*\{/.test(CSS))
check('含 paper stack（::before/::after）', /\.memory-book-portal::before/.test(CSS) && /\.memory-book-portal::after/.test(CSS))
check('含 spine', /\.memory-book-portal-spine/.test(CSS))
check('含 cover', /\.memory-book-portal-cover/.test(CSS))
check('含接触阴影（非整块漂浮 Card shadow）', /0 1px 1px rgba\(51, 43, 40, 0\.10\)/.test(CSS) && /0 4px 8px -3px/.test(CSS))
check('含 press 轻反馈（translateY 非 scale）', /translateY\(1px\)/.test(CSS) && !/scale\(\.9\d\)/.test(CSS))
check('含 reduced-motion 降级', /@media \(prefers-reduced-motion: reduce\)/.test(CSS) && /\.memory-book-portal-cover/.test(CSS))
